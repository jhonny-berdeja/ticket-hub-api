 ...
 * lookup-then-authorize flow. Its two public entry points
 * (`findAnsibleByNumber`/`findDatabaseByNumber`) are deliberately written
 * out in full below, NOT factored into one shared generic helper, even
 * though they're nearly identical — see `project-structure-conventions.md`,
 * "Handlers en el service general vs. service dedicado". The caller now
 * states which table to query explicitly (no prefix parsing happens
 * here), so each method queries only its own table and, exactly as
 * before, never leaks whether a "not found" was a missing number or a
 * found-but-not-authorized ticket. `isAdmin` stays as one small shared
 * free function: it's a trivial one-line auth check, not a "flow".
 */
@Injectable()
